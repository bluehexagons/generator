...
function nextSeed() {
  ...
  switch (parseInt(seedMode)) {
    case 0:
      ...
      break;
    case 1:
      if (seedMode - Math.floor(seedMode) > 0)
        nseed = seed + seedMode - Math.floor(seedMode);
      else nseed = seed + 0.1;
      if (nseed > 1) {
        nseed = 0.9999999999999999;
        seedMode = 2 + seedMode - Math.floor(seedMode);
    ...
    case 2:
      ...
      if (nseed < 0) {
        nseed = 0;
        seedMode = 1 + seedMode - Math.floor(seedMode);
        $("seedmode").value = seedMode;
      }
      break;
    default:
...
function pixel0() {
  var pixcol =
    Math.random() * 256 * 256 * 256 * (1 - x / w) +
    seed * 256 * 256 * 256 * (x / w);
  return (
    "rgb(" +
...
function pixel4() {
  var scale =
    Math.pow(Math.pow(x - w, 2) + Math.pow(y - h, 2), 1 / 2) /
    Math.pow(Math.pow(w, 2) + Math.pow(h, 2), 1 / 2);
...
function pixel7() {
  var scale =
    ...
    Math.pow(Math.pow(w, 2) + Math.pow(h, 2), 1 / 2);
  var pixcol =
    ((seed / 2) * 256 * 256 * 256 * scale * 2 * ((x / w) * (y / h))) / 256;
